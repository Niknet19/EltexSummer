...
void modify_permissions(mode_t *mode, const char *command) {
  ...
  if (op == '+')
    ...
    *mode &= ~bit;
    ...
    if (who == 'u') *mode &= ~S_IRWXU;
    if (who == 'g') *mode &= ~S_IRWXG;
    if (who == 'o') *mode &= ~S_IRWXO;
    if (who == 'a') *mode = 0;
    *mode |= bit;
  }
  ...
  while (1) {
    ...
    switch (choice) {
      ...
      case 3:
        if (!file_loaded) {
          printf(
              "Сначала введите права доступа (пункт 1) или имя файла (пункт "
              "2).\n");
          break;
        }
