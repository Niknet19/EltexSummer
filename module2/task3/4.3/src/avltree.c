...
AvlTree* insert_node(AvlTree* p, void* new_value,
...
AvlTree* dfs(AvlTree* p, void* value, int (*compare)(void*, void*)) {
  if (p == NULL || compare(p->value, value) == 0) {
    return p;
  }
  AvlTree* l = search_node(p->left, value, compare);
  AvlTree* r = search_node(p->right, value, compare);
  return l != NULL ? l : r;
}
...
AvlTree* remove_min(
    AvlTree* p)  // удаление узла с минимальным ключом из дерева p
{
...
// int contact_compare_with_str(const char* str, Contact* c2) {
//   return strcmp(str, c2->name);
// }
