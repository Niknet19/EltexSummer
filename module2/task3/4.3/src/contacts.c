...
bool delete_contact(ContactList *list, size_t id_to_delete) {
  ...
  delete_node(list->contacts, (size_t)id_to_delete, contact_compare_with_id);
  list->size--;
  if (list->size == 0) {
    // free(list->contacts);
    list->contacts = NULL;
...
bool add_contact(ContactList *list, char *firstname, const char *lastname,
  ...
  if (list->contacts =
          insert_node(list->contacts, newContact, contact_compare_by_id)) {
    list->size++;
    return true;
  } else
    return false;
...
bool update_contact(ContactList *list, size_t id_to_update, char *firstname,
  ...
  Contact *contact_ptr = (Contact *)search_node(list->contacts, id_to_update,
...
void print_contact_list(ContactList *list) {
  ...
  print_tree(list->contacts, 0, print_contact);
}
...
ssize_t find_contact_by_name(ContactList *list, Contact **contact_to_find,
  ...
  AvlTree *node = dfs(list->contacts, firstName, contact_compare_by_name_str);
  Contact *found = NULL;
  if (node != NULL) found = node->value;
